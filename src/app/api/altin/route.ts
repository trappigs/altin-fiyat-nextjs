...
async function getSaglamogluData() {
  try {
    ...
    if (usdItem) {
      ...
      console.warn('USD item bulunamadı!');
    }
    ...
    if (sarItem) {
      ...
      console.warn('SAR item bulunamadı!');
    }
...
function hesaplaVeListele(
  ...
  console.log('Hesaplama inputları:', { usdAlis, usdSatis, eurAlis, eurSatis, sarAlis, sarSatis });
...
export async function GET(request: Request) {
  try {
    ...
    if (cache && (now - cache.lastUpdate) < CACHE_DURATION) {
      ...
      console.log('Cache.data array check:', Array.isArray(cache.data) ? 'ARRAY' : 'NOT ARRAY');
    ...
    console.log('Response data type:', typeof urunListesi);
    console.log('Response data is array:', Array.isArray(urunListesi));
